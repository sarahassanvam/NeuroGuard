...
#reading the csv, building a deep learning modeel and training and evaluation metrics
...
MODEL_OUT = OUT_DIR / "detector_cnn_only.pt"
SCALER_OUT = OUT_DIR / "scaler_cnn_only.pkl"
RESULTS_OUT = OUT_DIR / "results_cnn_only.txt"
...
class CNN_Only(nn.Module):
    #simple CNN model for local pattern detection
    #this model only uses convolutional layers to detect short term patterns in packet sequences
    def __init__(self, feat_dim):
        ...
        self.fc = nn.Sequential(
            ...
            nn.Flatten(),
            #flatten all the cnn features into one long vector
            nn.Linear(128 * SEQ_LEN, 128),
            #compress the flattened features
            #128 * SEQ_LEN because we have 128 channels and SEQ_LEN timesteps
            nn.ReLU(),
...
def scale_features(X, scaler=None, fit=False):
    ...
    if fit:
        ...
        #the scaler will adjust using values
        #means that the model indirectly sees test distribution
        #so the evaluation becomes fake high accuracy
...
def main():
    ...
    model = CNN_Only(feat_dim).to(DEVICE)
    #build the model then move it to cpu/gpu (as our base rule)
