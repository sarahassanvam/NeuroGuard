#cnn + multi-head attention model
import os
...
# the full labeled packets csv
MODEL_OUT = OUT_DIR / "detector_cnn_attention.pt"
SCALER_OUT = OUT_DIR / "scaler_cnn_attention.pkl"
RESULTS_OUT = OUT_DIR / "results_cnn_attention.txt"
...
class CNN_Attention(nn.Module):
    #CNN for local patterns and the multi-head attention to focus on important packets
    #this model combines convolutional layers with multi-head attention mechanism
    def __init__(self, feat_dim, num_heads=4):
        ...
        # multi-head attention mechanism to focus on the most important packets
        # 128 is the hidden dimension from CNN output
    ...
    def forward(self, x):
        ...
        x = x.transpose(1, 2)
        # change back to (B, T, 128) for attention
        # attention expects (batch, seq_len, features)
        x = self.multi_head_attn(x)
        # multi-head attention focuses on important packets from multiple perspectives
        # output shape: (B, 128)
...
def main():
    ...
    model = CNN_Attention(feat_dim, num_heads=4).to(DEVICE)
    ...
    print("\n=== CNN + MULTI-HEAD ATTENTION - TEST METRICS ===")
    ...
    with open(RESULTS_OUT, "w") as f:
        f.write("=== CNN + MULTI-HEAD ATTENTION - TEST METRICS ===\n")
